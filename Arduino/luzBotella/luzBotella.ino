...
#define BUTTON_PIN   13    // Digital IO pin connected to the button.  This will be
...
void startShow(int i) {
  switch(i){
    case 0: todo(200);    // Black/off
            break;
    case 1:  mitad(200); // Blue
            break;
  }
}

// Fill the dots one after the other with a color
void mitad( uint8_t wait){
  strip.clear();
  strip.show();
  strip.fill(white, 14, 16);
  strip.show();
  delay(wait);
}

void todo(uint8_t wait) {
    strip.fill(white,0,0);
    strip.show();
    delay(wait);
}
